...
export default function CategoryPage() {
  const [categoriesPageData, setCategoryPageData] = useState<CategoryType[]>([])
  ...
  async function fetchCategoryServer(pageNumber = 1) {
    setLoading(true)
    try {
      ...
      console.error('Erro ao buscar categoriaes:', err)
      toast.error('Erro ao carregar categoriaes')
      setCategoryPageData([])
      setTotalPages(0)
      setTotalElements(0)
    } finally {
  ...
  async function fetchAllForSearch(): Promise<CategoryType[]> {
    setLoading(true)
    try {
      const responseCategory = new ProviderService()
      const resp = await responseCategory.getAllProvider(10000, 1)
    ...
    const t = setTimeout(() => {
      ...
      let mounted = true
        ; (async () => {
          ...
          if (!mounted) return
          const filtered = all.filter((p: CategoryType) => p.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  useEffect(() => {
    if (filteredCategory === null) void fetchCategoryServer(page)
  }, [page, filteredCategory])

  const displayedCategory = useMemo(() => {
    if (filteredCategory !== null) {
      const start = (page - 1) * PAGE_SIZE
      return filteredCategory.slice(start, start + PAGE_SIZE)
    }
    return categoriesPageData
  }, [filteredCategory, categoriesPageData, page])
  ...
  async function handleDelete(id: string) {
    ...
    toast.success('Categoria removido (simulado)')
    void fetchCategoryServer(page)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold">Gerir Categoriaes</h2>
